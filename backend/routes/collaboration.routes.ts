...
export async function addCollaboratorRoute(
  req: Request,
  params: { id: string }
  ...
  try {
    ...
    const collab = await CollaborationController.addCollaborator(
      params.id,
      userId,
      user
    );
    ...
    const status =
      error.message.includes("permission") || error.message.includes("Only")
        ? 403
        : 400;
...
export async function removeCollaboratorRoute(
  req: Request,
  params: { id: string }
): Promise<NextResponse> {
  try {
