...
| **Distributed Message Broker (Kafka) & Redis Cluster** | Introducing Apache Kafka / RabbitMQ for event streaming and a standalone Redis cluster for caching and queue pub/sub. | **PostgreSQL ACID transactions, composite B-tree indexes, advisory locking & lightweight in-memory caching.** | Massive infrastructure overkill for this stage. Introducing Kafka and Redis adds distributed failure modes, operational maintenance, and cache invalidation complexity across frequent ticket status mutations. PostgreSQL ACID transactions with advisory locking (`pg_try_advisory_xact_lock`) and composite indexes handle hundreds of requests/sec with sub-10ms query times and zero data drift. Starting with a clean, robust relational foundation keeps deployment trivial; if throughput demands 100k+ tickets or multi-region scale in the future, the decoupled architecture can cleanly upgrade to Redis and Kafka without rewriting domain logic. |
